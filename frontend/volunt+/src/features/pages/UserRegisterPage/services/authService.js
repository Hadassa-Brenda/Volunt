...
export async function register(bairro, cep, city, email, profileType, confirmPassword, password, whatsapp, fullName, state, acceptTerms) {
  if (!email || !password || password !== confirmPassword || !acceptTerms) {
    throw new Error("Dados de cadastro inválidos");
  }
  const user = { bairro, cep, city, email, profileType, whatsapp, fullName, state };
